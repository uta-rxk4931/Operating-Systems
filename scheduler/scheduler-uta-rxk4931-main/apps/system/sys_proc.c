...
int main() {
    ...
    while (1) {
        ...
        case PROC_TICKETS:  //Added for ticket functionality-case 1 changed
            grass->proc_set_ticket(req->argv[0][0]);
            grass->sys_send(sender, (void*)reply, sizeof(reply));
            break;

        case PROC_KILL: //Added for kill functionality
           {
